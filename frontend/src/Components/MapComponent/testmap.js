...
/*const MapComponent = ({ data = [], searchQuery, clientsData = [], nearbyWorkers, setIsMapLoading,activeChats,setActiveChats,handleFreelancerDecision }) => {
  ...
  /*useEffect(() => {
    const handleFreelancerDecision = ({ clientId, workerId, decision }) => {
      const currentClientId = localStorage.getItem('id');
      if (clientId === currentClientId) {
        setResponseStatus(prev => ({ ...prev, [workerId]: decision }));
    ...
    const handleAvailabilityChanged = ({ freelancerId, isAvailable }) => {
      setFilteredData(prev => 
        prev.filter(worker => 
          worker.id !== freelancerId || isAvailable
        )
      );
      
      if (selectedWorker?.id === freelancerId && !isAvailable) {
        setSelectedWorker(null);
      }
    };

    socket.on("freelancer_decision", handleFreelancerDecision);
    socket.on("availability_changed", handleAvailabilityChanged);

    return () => {
      socket.off("freelancer_decision", handleFreelancerDecision);
      socket.off("availability_changed", handleAvailabilityChanged);
    };
  }, [navigate, selectedWorker]);*
  ...
  const handleMarkerClick = (worker) => {
    ...
    setSelectedWorker({
    ...worker,
    name: `${worker.name} ${worker.surname}` // Ensure full name
  });
  ...
  const handleMapLoad = () => {
    ...
    setTimeout(() => {
      if (mapRef.current) {
        mapRef.current.invalidateSize(); // Corrects tile rendering
      }
    }, 300);
  };
    ...
    <div className={styles.mapWrapper}>
      ...
      >
        ...
        <TileLayer
          ...
          minZoom={3}
          detectRetina={true}
          updateWhenIdle={false} // Important for smooth loading
        ...
        {/* Render markers for filtered workers *}
        {isMapReady && filteredData.map((worker) => (
          worker?.latitude && worker?.longitude && (
            <Marker
              ...
              eventHandlers={{
                click: () => /*setSelectedWorker(worker)* handleMarkerClick(worker)
              }}
            >
              <Popup>
